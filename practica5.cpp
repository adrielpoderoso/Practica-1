...
Camera camera;
//AGREGAR LOS MODELS: Model Goddard_M;
Model CarroBase_M;
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::translate(model, glm::vec3(1.4f, 0.42f, 0.83f));
		modelaux=model;//chekpoint
		...
		model = modelaux;//libera chekpoint
		color = glm::vec3(1.0f, 0.0f, 1.0f);
		model = glm::translate(model, glm::vec3(0.0f, 0.04f, -1.82f));
		modelaux = model;//chekpoint
		...
		model = glm::translate(model, glm::vec3(-0.3f, 0.5f, -0.94f));
		modelaux = model;//chekpoin
