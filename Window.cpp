...
Window::Window(GLint windowWidth, GLint windowHeight)
{
	...
	capotaje = 0.0f;
	cajuela = 0.0f;
	trasladohelicoptero = 0.0f;
	luzcofre = false;
	luzModeloEncendida = true;
...
int Window::Initialise()
{
	...
	//CREAR VENTANA
	mainWindow = glfwCreateWindow(width, height, "PRACTICA 7 TRIGUEROS LOPEZ HECTOR ADRIAN", NULL, NULL);
...
// INICIO DE CAJUELA
	...
	//INICIO DE CAPOTAJE
	if (key == GLFW_KEY_T) {  // Abrir capotaje
		if (theWindow->capotaje < 45.0f) {
			theWindow->capotaje += 2.0f;  
			theWindow->luzcofre = theWindow->capotaje / 45.0f;  // Intensidad proporcional (0-1)
		}
	}
	else if (key == GLFW_KEY_Y) {  // Cerrar capotaje
		if (theWindow->capotaje > 0.0f) {
			theWindow->capotaje -= 2.0f;
			theWindow->luzcofre = theWindow->capotaje / 45.0f;
		}
	}
	//FIN DE CAPOTAJE
	
	//INICIO DEl HELICOPTERO
	if (key == GLFW_KEY_H) {
		theWindow->trasladohelicoptero += 2.0f;
	}
	if (key == GLFW_KEY_J) {
		theWindow->trasladohelicoptero -= 2.0f;
	}
	...
	//INICIO LUZ DE CASA ZIM
	if (key == GLFW_KEY_U && action == GLFW_PRESS) {
		theWindow->luzModeloEncendida = !theWindow->luzModeloEncendida;
	}
	//FIN LUZ DE CASA ZIM
